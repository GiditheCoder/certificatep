...
//   const { name, address, lga, nativeTown,email, stateOfOrigin,  image, approvedDate, certificateId } = state || {};
//   console.log("Certificate State:", state);
//   const Admin = JSON.parse(localStorage.getItem("user"));
//   const [menuOpen, setMenuOpen] = useState(false);
//   const [signatory, setSignatory] = useState(null);

//   // get signature using lga to get information such as chairmanName , secretaryName and signature
...
//         `https://lgacertificate-011d407b356b.herokuapp.com/api/v1/signatory/${encodeURIComponent(lga)}`
//       );
//       console.log("✅ Signatory data:", response.data);
//       setSignatory(response.data.data || response.data); // adjust depending on API structure
//     } catch (error) {
...
//     <p className="text-xs mt-1 font-semibold">{signatory?.secretaryName || "Secretary"}</p>
//   </div>
...
//     )}
//     <div className="border-t border-gray-400 w-28 sm:w-32 mx-auto"></div>
//     <p className="text-xs mt-1 font-semibold">{signatory?.chairmanName || "Chairman"}</p>
//   </div>
// </div>


//           <p className="text-xs font-medium text-black mt-8 break-words relative z-10">
//             Certificate ID: {certificateId || "N/A"}
//           </p>

//            {/* <p className="text-xs font-medium text-black mt-1 break-words relative z-10">
//            Admin Email: {email || "N/A"}
//           </p> */}
//         </div>
...
//           <button
//             onClick={() => navigate("/officialscreen")}
...
//   <button
//     onClick={() => {
//       // Check where it came from; default to dashboard
...
const ApprovedCertificate = () => {
  ...
  // Determine which LGA to use
  const effectiveLga = stateOfOrigin?.toLowerCase() === "ogun" ? lga : lgaOfResident;

  // Fetch signatory data based on effective LGA
  useEffect(() => {
    const fetchSignatory = async () => {
        ...
        >
          ...
          <div className="text-left leading-relaxed text-gray-800 text-xs sm:text-sm md:text-base space-y-3 sm:space-y-4 relative z-10">
            <p>
              This is to certify that from enquiry made,{" "}
              <span className="font-semibold underline">{name || "Full Name"}</span> of{" "}
              <span className="font-semibold underline">{address || "Full Address"}</span> is a native of{" "}
              <span className="font-semibold underline">{lga || "Local Government Area"}</span> of{" "}
              <span className="font-semibold underline">{stateOfOrigin || "State"}</span> State, Nigeria.
            </p>
            <p>
            ...
            <p>
              ...
              <span className="font-semibold underline">{effectiveLga || "Local Government Area"}</span> on{" "}
              <span className="font-semibold underline">
                {approvedDate
                  ? (() => {
                      const date = new Date(approvedDate);
