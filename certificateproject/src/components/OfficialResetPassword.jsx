...
const OfficialResetPassword = () => {
  ...
  setMessage(
    error.response?.data?.message || "Failed to send OTP. Please try again later."
  );
} finally {
      setResending(false);
    }
  ...
  return (
    ...
    >
      <div className="w-full max-w-sm space-y-6">
        ...
        {errors.otp && <p className="text-red-500 font-medium text-sm text-center">{errors.otp}</p>}
        ...
        <div className="text-center font-medium text-gray-600 text-sm mt-2">
          {timeLeft > 0 ? (
            <p className="font-medium">
              OTP expires in{" "}
              <span className="text-green-600 font-semibold">{formatTime(timeLeft)}</span>
            </p>
          ) : (
            <button
...
export default OfficialResetPassword;
